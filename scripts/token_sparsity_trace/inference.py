...
def inference(model, tokenizer, prompt, max_length, device, hidden_output_path=None):
    # Process prompt
    ...
    output_token_size = output_seq.shape[1]  # Total tokens in the generated sequence
    input_token_size = inputs.input_ids.shape[1]  # Tokens in the input sequence
    generated_token_size = (
        output_token_size - input_token_size
    ...
    print(
        ...
        tokenizer.batch_decode(
            output_seq, skip_special_tokens=True, clean_up_tokenization_spaces=False
        )[0],
    )

    if hidden_output_path:
        # Save the hidden states to the specified directory
        hidden_states = outputs["hidden_states"]
        # Save hidden states in safetensors format
        hidden_states_dict = {
            f"layer_{i}": hidden_states[i][0].cpu() for i in range(len(hidden_states))
        }
        hidden_states_file = hidden_output_path
        save_file(hidden_states_dict, hidden_states_file)
